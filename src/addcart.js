...
function Cart(){
  ...
  const Delete=(id)=>{
    fetch("https://ecommerce-app-api-39o5.onrender.com/brand/"+id,{
        method:"DELETE"
    })
 ...
 <div className="five">
  ...
  <tbody class="table-group-divider">
    ...
    {data.map((one) => (
      ...
      <td scope="row">{one.brand}</td>
      <td scope="row">{one.model}</td>
      <td scope="row">{one.year}</td>
      <td scope="row" style={{fontSize:"20px",color:"red"}}> ${one.price}</td>
    
      <td> <button id="a" className="btn btn-primary"  onClick={()=>{process(one.id)}} >Details</button></td>
      <td>
